...
public class AddNewWordActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mViewModel=new ViewModelProvider(this,ViewModelProvider.AndroidViewModelFactory.getInstance(this.getApplication())).get(AddNewWordViewModel.class);
    ...
    private void saveWord() {
        String word=wordEditText.toString().trim();
        String meaning=meaningEditText.toString().trim();
        String type=typeEditText.toString().trim();

        Words wordObject=new Words(word,meaning,type);
        if(word.isEmpty() ||meaning.isEmpty()|| type.isEmpty()){
            Toast.makeText(this, "please find all fields", Toast.LENGTH_SHORT).show();
            return;
        }

        if(editMode){
            wordObject.setId(mId);
            mViewModel.updat(wordObject);
        }else{
